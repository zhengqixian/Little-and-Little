  ...
  <div class="page-cart">
      ...
      <div class="main" v-show="carts.length>0">
          <table class="receive" width="100%">
              <tr>
                  <th width="28%">
                      收<span class="text-hidden">a</span>贷<span class="text-hidden">a</span>人
                  </th>
              ...
              <tr>
                  <th>电<span class="text-hidden">中</span><span class="text-hidden">中</span>话</th>
                  <td>{{user.selectSite.phone}}</td>
              </tr>
              <tr>
                  <th>收货地址</th>
                  <td>{{user.selectSite.city}}{{user.selectSite.site}}{{user.selectSite.datailSite}}</td>
              </tr>
          </table>
...
computed:{
    ...
    total(){
        return this.$store.gettters.total
    },
    checkedAll(){
        let checkedAll = true
...
methods:{
    ...
    changeCheckedAll(){
        if(this.checkedAll){
            //全部取消
            this.$dtore.dispatch('checkedAllFales')
            .then(res => {
                this.$msg('提示',res.msg)
            })
        }else{
